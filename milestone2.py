...
if __name__=="__main__":
    ...
    df.to_csv("Milestone2_Seniment_Results.csv",index=False)
    print("Milestone 2 Completed Successfully")
    print(df[["clean_feedback","sentiment","confidence_score"]].head(5))

    sentiment_count=df['sentiment'].value_counts()
    plt.figure(figsize=(8,5))
    colors=['green','red','gray']
    sentiment_count.plot(kind="bar",color=colors)
    plt.title("How Customers Feel - Sentiment Analysis",fontsize=14)
